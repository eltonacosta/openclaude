...
- Search existing [issues](https://github.com/eltonacosta/orbit-code/issues) and [discussions](https://github.com/eltonacosta/orbit-code/discussions) before opening a new thread.
- Check [open pull requests](https://github.com/eltonacosta/orbit-code/pulls) for work that overlaps with your contribution. If a PR already exists that addresses the same change, open an issue or discussion first to align on direction — duplicate PRs may be closed without review.
...
- Search [open pull requests](https://github.com/eltonacosta/orbit-code/pulls) for related work
- Check [closed pull requests](https://github.com/eltonacosta/orbit-code/pulls?q=is%3Apr+is%3Aclosed) to see if similar work was previously addressed or declined
...
git fetch https://github.com/eltonacosta/orbit-code.git main
...
- Fetching upstream `main` by URL avoids assuming that a fork checkout's `origin` points at eltonacosta/orbit-code. `FETCH_HEAD` is the fetched upstream tip. The scan deliberately uses local `HEAD` so it includes commits that have not been pushed yet; CI uses the pushed PR head SHA after the push.
