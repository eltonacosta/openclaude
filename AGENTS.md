...
This repo keeps a queryable knowledge graph at `.graphify/` (28k nodes,
584 communities, generated by [`@sentropic/graphify`](https://www.npmjs.com/package/@sentropic/graphify)).

- Prefer `graphify query "<pergunta>"`, `graphify path "<A>" "<B>"`,
  or `graphify explain "<conceito>"` over raw `grep` for architecture
  questions; use `graphify summary` for first-hop orientation.
- Read `.graphify/GRAPH_REPORT.md` only for broad architecture review.
- After modifying code, run `graphify update` (code-only, no LLM) or
  `graphify hook-rebuild` to keep the graph current; run
  `graphify check-update` if results look stale.
- Never commit `.graphify/branch.json`, `.graphify/worktree.json`,
  `.graphify/needs_update` or `.graphify/cache/` (see `.gitignore`);
  run `graphify portable-check .graphify` before committing graph artifacts.
- `graphify` CLI setup: `npm install -g @sentropic/graphify`
  (requires `~/.npm-global/bin` on `PATH`).
