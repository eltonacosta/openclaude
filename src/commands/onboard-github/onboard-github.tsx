...
function OnboardGithub(props: {
  ...
  const finalize = useCallback(
    async (
      ...
      onDone(
        successMsg +
          'Copilot token and OAuth token stored in secure storage (Windows/Linux: ~/.orbitcode/.credentials.json, macOS: Keychain fallback to ~/.orbitcode/.credentials.json); user settings updated. Restart if the model does not switch.',
        { display: 'user' },
      )
    },
      ...
      if (!activated.ok) {
        setErrorMsg(
          `Key saved, but settings were not updated: ${activated.detail ?? 'unknown error'}. ` +
            `Add env CLAUDE_CODE_USE_GITHUB=1 and OPENAI_MODEL to ~/.orbitcode/settings.json manually.`,
        )
        setStep('error')
        return
      }
