...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@eltonacosta/openclaude structure
...
describe('package consumer types', () => {
  test('SDK types compile for external consumer with skipLibCheck:false', () => {
    ...
    writeFileSync(
      ...
      [
        ...
        `  SDKSession,`,
        `  SDKSessionOptions,`,
        `} from '@eltonacosta/openclaude/sdk'`,
        ``,
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@eltonacosta/openclaude/sdk'`,
        ``,
  ...
  test('result usage standard counters are required numbers', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKResultMessage } from '@eltonacosta/openclaude/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@eltonacosta/openclaude/sdk'`,
        ``,
  ...
  test('control initialize response accepts generated model and account values', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKControlInitializeResponse, ModelInfo } from '@eltonacosta/openclaude/sdk'`,
        ``,
